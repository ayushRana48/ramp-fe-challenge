...
/*Fixed bug 1 by removing position fixed, and setting position to 0 0 since it is child of root */
...
//fixed bug 2 by adding htmlFor in label
...
//Fixed bug 3 by checking if empty employee load all else load id in App.tsx
...
//fixed bug 4 by creating use state that appends new items to list of visible transactions and setting list as data attribute in usePaginatedTransactions.ts
...
//fixed bug 5 by not showing "loading" if transactions are loading in InputSelect index.ts
...
//fixed bug 6 by disabling view more button if on employee page and checking if next page is null and if it is disable view more for all emplyee page
...
//fix bug 7 by creating known Changes state in App.tsx which gives a list of the most updated transactions that have been changed from default value updates transaction to match known Changes, then transaction prop changes, which re renderes Transaction Pane,
